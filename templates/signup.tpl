...
{assign var=groupid value="36"}
{assign var=namegroup value="Знакомства"}
{* Конец переменных для групп *}
...
<form action='signup.php' method='POST'>
  {* LOOP GROUPS *}
<div class='signup_header'>Группы</div>
<strong>Поставьте галочку рядом с группой если хотите в неё вступить</strong><br>
<input type="checkbox" class="checkbox" name='field_{$groupid}' id='label_{$groupid}_1' value='{$namegroup}'>{$namegroup}<br><br>
  {* LOOP THROUGH TABS *}
...
{if $tabs[tab_loop].fields[field_loop].field_id != $groupid}
      <tr>
