     ...
     <div class="flexBox clearFix no_padding" style="margin-left:-7px">
      <div id = "fBox">
        ...
        <table class='friendTable' cellpadding='0' cellspacing='0' height='100%'><tr>
          ...
          {assign var=cont value="`$cont+1`"}<a href='{$url->url_create('profile',$friends[friend_loop]->user_info.user_username)}'>
          ...
          {$friends[friend_loop]->user_info.user_username}</a>
          </td>
...
<b class="nc"><b class="nc1"><b></b></b><b class="nc2"><b></b></b></b>
     ...
     <input style="width:7.4em; float:right; height:1.3em; height:25px;" type='submit' id='comment_submit' class="buttonRow" class="clearFix" value='Отправить'>
      ...
      {section name=comment_loop loop=$comments}
        ...
        <td class='profile_item1' width='80'>
          {if $comments[comment_loop].comment_author->user_info.user_id != 0}
            <a href='{$url->url_create('profile',$comments[comment_loop].comment_author->user_info.user_username)}'><img src='{$comments[comment_loop].comment_author->user_photo('./images/nophoto.gif')}' class='photo' border='0' width='{$misc->photo_size($comments[comment_loop].comment_author->user_photo('./images/nophoto.gif'),'75','999','w')}'></a> {if $comments[comment_loop]->user_info.user_online == 1}Online{/if} 
          {else}
        ...
        <td class='profile_item2'>
          ...
          <td class='profile_comment_author' align='right' nowrap='nowrap'>&nbsp;{if $comments[comment_loop].comment_author->user_info.user_id != 0}<a href='{$url->url_create('profile', $comments[comment_loop].comment_author->user_info.user_username)}#comments'>Ответить</a>&nbsp;|&nbsp;<a href='user_messages_new.php?to={$comments[comment_loop].comment_author->user_info.user_username}'>{$profile34}</a>{/if}</td>
          </tr>
          <tr>
