...
class se_user {
	    ...
	    if($tabs_only == 0) {
	      ...
	      while($field_info = $database->database_fetch_assoc($fields)) {
		    ...
		    // FORMAT VALUE FOR FORM
	              ...
	              $year_array = Array();
	              $year_count = 1;
	              $current_year = $datetime->fdate("Y", time());
	...
	function user_friend_list($start, $limit, $direction = 0, $friend_status = 1, $sort_by = "se_users.user_dateupdated DESC", $where = "", $friend_details = 0) {
      ...
      if(in_array($friend->user_info[user_username], $online_users_array)) { 
        $friend->user_info[user_online] = 1; 
      } else { 
        $friend->user_info[user_online] = 0; 
      } 
